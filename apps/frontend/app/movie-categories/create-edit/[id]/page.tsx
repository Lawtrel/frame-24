...
export default function CreateEditMovieCategoryPage() {
    const { id } = useParams();
    ...
    useEffect(() => {
        ...
        const loadData = async () => {
            try {
                if (isEdit) {
                    const category = await movieCategoriesService.getById(id as string, token);
                    setDefaultValues({
                        name: category.name,
    ...
    const onSubmit = async (data: CategoryFormData) => {
        ...
        try {
            if (isEdit) {
                await movieCategoriesService.update(id as string, data, token);
                alert('Categoria atualizada com sucesso!');
            } else {
                await movieCategoriesService.create(data, token);
                alert('Categoria criada com sucesso!');
            }
            router.push('/movie-categories');
        } catch (error) {
    ...
    return (
        <FormLayout
            title={isEdit ? 'Editar Categoria de Filme' : 'Nova Categoria de Filme'}
