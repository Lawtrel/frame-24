              ...
              <li key={ticket.id} className="flex flex-wrap items-center justify-between gap-2 rounded-[var(--radius-md)] border border-border p-3">
                <div>
                  <p className="font-medium">
                    {ticket.movie?.title || `#${ticket.ticket_number}`}
                  </p>
                  <p className="text-xs text-foreground-muted">
                    #{ticket.ticket_number} · Pedido {ticket.sale.sale_number} ·{" "}
                    {formatDateTimeInTimeZone(ticket.showtime?.start_time || ticket.sale.sale_date)}
                  </p>
                  {ticket.showtime ? (
                    <p className="text-xs text-foreground-muted">
                      {ticket.showtime.cinema}
                      {ticket.showtime.room ? ` · ${ticket.showtime.room}` : ""}
                    </p>
                  ) : null}
                </div>
                <div className="flex items-center gap-2">
                  ...
                  <Button asChild size="sm" variant="secondary">
                    <Link href={withTenantPath(pathname, `/perfil/ingressos/${ticket.id}`)}>{copy("profileTicketsOpen")}</Link>
                  </Button>
                </div>
              </li>
            ))}
