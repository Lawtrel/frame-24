...
export class PosSessionResponseDto extends createZodDto(
  ...
  @ApiProperty({ description: 'ID da sessão PDV', example: '243244130367442946' })
  id!: string;
  ...
  @ApiPropertyOptional({ description: 'Nome legível do status', example: 'Aberta' })
  status_name?: string;

  @ApiProperty({ description: 'Número da sessão', example: 'PDV-20260523-001' })
  session_number!: string;

  @ApiProperty({ description: 'Valor de abertura (fundo de troco)', example: 200.0 })
  opening_amount!: number;

  @ApiProperty({ description: 'Total de retiradas em espécie', example: 0 })
  cash_withdrawn!: number;

  @ApiProperty({ description: 'Valor contado no fechamento', nullable: true, example: 3450.75 })
  cash_counted!: number | null;

  @ApiProperty({ description: 'Diferença apurada no fechamento', nullable: true, example: -2.5 })
  ...
  @ApiProperty({ description: 'Total recebido (todas formas de pagamento)', example: 3080.0 })
  total_received_amount!: number;

  @ApiProperty({ description: 'Total de troco devolvido', example: 70.0 })
  total_change_given!: number;

  @ApiProperty({ description: 'Data/hora de abertura', example: '2026-05-23T08:00:00.000Z' })
  opened_at!: string;

  @ApiProperty({ description: 'Data/hora de fechamento', nullable: true, example: '2026-05-23T23:30:00.000Z' })
