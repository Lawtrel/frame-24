...
export class AccountsPayableRepository {
    ...
    async update(
        ...
        if (dto.competence_date) data.competence_date = new Date(dto.competence_date);
    ...
    async updateStatus(id: string, status: string, paid_amount: number, remaining_amount: number) {
        return this.prisma.accounts_payable.update({
            where: { id },
