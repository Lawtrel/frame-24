...
export const CreateCompanySchema = z.object({
  corporate_name: z.string().min(3, 'Razão social deve ter no mínimo 3 caracteres').max(200, 'Razão social deve ter no máximo 200 caracteres'),
  cnpj: z
    .string()
    .regex(/^\d{14}$|^\d{2}\.\d{3}\.\d{3}\/\d{4}-\d{2}$/, 'CNPJ inválido')
    .transform((val) => val.replace(/\D/g, '')),
  tax_regime: z.nativeEnum(tax_regime_type, { message: 'Regime tributário inválido' }).optional(),
  trade_name: z.string().max(200, 'Nome fantasia deve ter no máximo 200 caracteres').optional(),
  ...
  website: z.string().url('URL inválida').max(200, 'URL muito longa').optional(),

  state_registration: z.string().max(20, 'Inscrição estadual muito longa').optional(),
  municipal_registration: z.string().max(20, 'Inscrição municipal muito longa').optional(),
